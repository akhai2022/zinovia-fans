...
# Security group: RDS (allow from ECS + additional shared projects)
resource "aws_security_group" "rds" {
  name_prefix = "${var.name_prefix}-rds-"
  description = "RDS: allow from ECS tasks and shared projects"
  vpc_id      = aws_vpc.main.id
  ingress {
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
    security_groups = concat([aws_security_group.ecs.id], var.additional_rds_ingress_security_group_ids)
  }
  egress {
    from_port   = 0
    to_port     = 0
